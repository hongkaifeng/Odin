...
# read corpus
with open(f'D:/mymodel-github/data/documents.json',encoding='utf-8') as f:
    corpus = json.load(f)
...
# read query
query = {}
with open(f'D:/mymodel-github/data/{query_file}',encoding='utf-8') as f:
    readin = f.readlines()
    for line in readin:
...
# search
res = {}
with open(f'D:/mymodel-github/data/{res_file}', 'w',encoding='utf-8') as fout:
    for qid in tqdm(query):
        ...
        for rank, idd in enumerate(topk):
            fout.write(qid +' Q0'+ ' ' + str(docid_list[idd]) + ' ' + str(rank) + ' ' + str(doc_scores[idd]) + ' bm25' + '\n')
