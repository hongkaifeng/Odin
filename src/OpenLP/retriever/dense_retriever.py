...
class Retriever:
                ...
                with torch.no_grad():
                    for k, v in batch.items():
                        if not isinstance(v, dict):
                            ...
                            #print(k)
                            for i in v:
                                i['input_ids']=torch.unsqueeze(torch.tensor(i['input_ids']).long(), dim=0).to(self.args.device)
                                i['attention_mask']=torch.unsqueeze(torch.tensor(i['attention_mask']).long(), dim=0).to(self.args.device)
                                i['edge']=i['edge'].to(self.args.device)
                            #return error
    ...
    def init_index_and_add(self, partition: str = None):
        ...
        #print(partitions)
        #print(os.path.join(self.args.output_dir, f"{self.args.retrieve_domain}.embeddings.corpus.rank.*"))
                    ...
                    for k, v in batch.items():
                        if not isinstance(v, dict):
                            ...
                            for i in range(len(v)):
                                for j in v[i]:
                                    #print(v)
                                    v[i][j]=torch.tensor(v[i][j]).to(self.args.device)
                            #print(v)
                            #return error
                            batch[k] = v
                        else:
