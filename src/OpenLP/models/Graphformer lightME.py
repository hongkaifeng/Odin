...
from transformers.models.bert.modeling_bert import BertSelfAttention, BertEmbeddings, BertPreTrainedModel, \
    BertAttention, BertIntermediate, BertOutput  # , BertLayer
from transformers.modeling_outputs import BaseModelOutputWithPoolingAndCrossAttentions
from transformers.pytorch_utils import apply_chunking_to_forward, find_pruneable_heads_and_indices, prune_linear_layer
from transformers.models.distilbert.modeling_distilbert import (
    DistilBertPreTrainedModel,  # 基类，存在
    MultiHeadSelfAttention,  # 注意力层（源码中 DISTILBERT_ATTENTION_CLASSES 的默认值）
    TransformerBlock,  # 编码器层（替代原 BertLayer/DistilBertLayer）
    Embeddings,  # 嵌入层（替代原 DistilBertEmbeddings）
    FFN  # 前馈网络（TransformerBlock 依赖）
)
...
class BertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.output = BertSelfOutput(config)  # 保留你原有的 BertSelfOutput（若无需修改）
...
class CLSAggregator(MessagePassing):
    def __init__(self):
        super().__init__(aggr='mean')  # 均值聚合
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        # 处理空输入情况
        if inputs.numel() == 0:
            return torch.zeros_like(self.x)  # 返回与输入同形状的零张量
...
class GraphBertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # 保持layer命名，与标准DistilBERT的transformer.layer一致
        self.layer = nn.ModuleList([TransformerBlock(config) for _ in range(config.num_hidden_layers)])
        # 自定义GraphSAGE层（无预训练权重，需训练）
        self.sagelayer = nn.ModuleList(
            [SingleLayerGraphSAGE(in_channels=config.hidden_size, out_channels=config.hidden_size) for _ in
             range(self.aggnum)]
        )

    # forward 方法逻辑不变，但需确保所有层引用正确（如 self.layer、self.sagelayer）
    def forward(self, hidden_states, attention_mask, graph, all_delet_belt):
        # 原 forward 逻辑不变，无需修改
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # 原层调用逻辑不变（self.layer[i] 对应标准模型的 transformer.layer[i]）
            if i > 0:
                if i in aggl:
                    ...
                    hidden_states, graph, all_delet_belt, mapping, attention_mask = remove_nodes(hidden_states, graph,
                                                                                                 all_delet_belt,
                                                                                                 max(nowdelete_mask - nowaggl,
                                                                                                     1), attention_mask)
                    ...
                    # 非聚合层，修复聚合时的_empty错误
                    num_nodes, seq_len, embed_dim = hidden_states.shape

                    # 确保graph是PyG格式的edge_index
                    if isinstance(graph, list):
                        edge_index = torch.tensor(graph, dtype=torch.long, device=hidden_states.device)
                    else:
                        edge_index = graph

                    # 创建聚合器（处理空消息情况）

                    aggregator = CLSAggregator().to(hidden_states.device)

                    # 提取所有节点的[CLS]嵌入 (确保张量有效)
                    ...
                    # 执行聚合（带错误处理）
                    #try:
                    #print(edge_index_with_self_loops)
                    if 1:
                        aggregated_cls = aggregator(edge_index_with_self_loops.long().to(hidden_states.device), cls_embeddings)
                    #except:
                        # 聚合失败时使用原始嵌入
                        #aggregated_cls = cls_embeddings

                    # 再次验证聚合结果有效性
                    if aggregated_cls.numel() == 0 or not hasattr(aggregated_cls, 'dim'):
                        aggregated_cls = cls_embeddings

                    # 更新隐藏状态中的[CLS]标记
                    hidden_states[:, 0] = aggregated_cls
            ...
            if self.output_attentions and len(layer_outputs) == 2:
...
class GraphFormers(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.config = config
        # 仅保留distilbert前缀的模块，与标准结构一致
        self.distilbert = nn.ModuleDict({
            'embeddings': Embeddings(config=config),  # 标准嵌入层
            'transformer': GraphBertEncoder(config=config)  # 自定义编码器（含GraphSAGE）
        })
    ...
    def forward(self,
        ...
        embedding_output = self.distilbert.embeddings(input_ids=input_ids)  # token embedding
        ...
        encoder_outputs = self.distilbert.transformer(
            embedding_output,
