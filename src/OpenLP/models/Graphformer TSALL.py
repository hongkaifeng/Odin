...
class SingleLayerGraphSAGE(torch.nn.Module):
    ...
    def forward(self, hidden_states, graph, nowaggl):
        for param in self.conv.parameters():
            nowdevice = param.device
            break
        if nowaggl == 10000000000:
            print("============================================")
        ...
        x = self.conv(limx.to(nowdevice), graph.long().to(nowdevice))  # 直接聚合邻居特征
...
class GraphBertEncoder(nn.Module):
    def __init__(self, config):
        print("现在是 TYPE V GraphBertEncoder")
        ...
        self.sagelayer = nn.ModuleList(
            [SingleLayerGraphSAGE(in_channels=768, out_channels=768) for _ in range(self.aggnum)])
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i > 0:
                if 1:
                    ...
                    # try:
                    if 1:
                        aggregated_cls = aggregator(edge_index_with_self_loops.to(hidden_states.device), cls_embeddings)
                    # except:
                    # 聚合失败时使用原始嵌入
