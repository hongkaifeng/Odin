...
class CLSAggregator(MessagePassing):
    def __init__(self, lambda_gate=0.5):  # 新增λ（控制邻居聚合强度，0<λ<1）
        super().__init__(aggr='mean')
        self.lambda_gate = lambda_gate  # 对应文档中的λ，默认0.5（平衡自身与邻居）

    def forward(self, edge_index, x):
        if x.numel() == 0:
            print("return x")
            return x
        # 1. 调用父类传播逻辑，获取邻居均值聚合结果（D^{-1}A·H^{(l)}）
        neighbor_agg = self.propagate(edge_index, x=x)
        # 2. 融合自身特征与邻居聚合结果：H = I·x + λ·neighbor_agg（文档核心公式）
        self_plus_neighbor = x + self.lambda_gate * neighbor_agg
        return self_plus_neighbor
    ...
    def aggregate(self, inputs, index, ptr=None, dim_size=None):
        if inputs.numel() == 0:
            print("zeroagg")
            # 修正：空输入时返回与inputs同形状的零张量（原代码self.x未定义）
...
class GraphBertEncoder(nn.Module):
    ...
    def forward(self,
        ...
        hidden_states, graph, all_delet_belt, mapping,attention_mask = remove_nodes(hidden_states, graph, all_delet_belt,1,attention_mask)
        #print(hidden_states.shape)
        #import error
        # Add last layer
        if self.output_hidden_states:
