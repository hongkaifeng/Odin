...
class GraphBertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, graph, all_delet_belt):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i >= aggl[0]:
                if i in aggl:
                    cls_emb = hidden_states[:, 1].clone()
                    station_emb = self.sagelayer[nowaggl](cls_emb, graph, nowaggl)
                    hidden_states[:, 0] = station_emb

                    hidden_states, graph, all_delet_belt, mapping, attention_mask = remove_nodes(
                        hidden_states, graph, all_delet_belt, max(nowdelete_mask - nowaggl, 1), attention_mask
                    )
