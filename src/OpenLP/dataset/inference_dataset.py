...
class InferenceDataset(Dataset):
    ...
    def _process_func(self, example):
        ...
        if 'text' in example:
            ...
            if isinstance(example['text'], str):
                ...
                tokenized = self.tokenizer.encode_plus(example['text'], truncation='only_first', padding='max_length',
                                                       max_length=self.max_len)

            tokenized['edge']=torch.tensor([[],[]]).long()
            return {"text_id": example_id, 'x1': tokenized}
        else:
        #if isinstance(example['text'], str):
            #print(example)
            #return error
            if isinstance(example['x1'], str):
                tokenized = self.tokenizer(example['x1'], padding='max_length', truncation=True, max_length=self.max_len)
            else:
                item = []
                for i in example['x1']:
                    item.append(self.tokenizer(i, padding='max_length', truncation=True,
                                           max_length=self.max_len))
                input_ids=[]
                attention_mask=[]
                token_type_ids=[]
                for i in item:
                    ...
                    if 'token_type_ids' in i:
                        ...
                        # 对于distilbert，可以使用全0的token_type_ids
                        token_type_ids.append([0] * len(i['input_ids']))
                    #token_type_ids.append(i['token_type_ids'])
                #return error
                G={'input_ids':torch.tensor(input_ids).long(),'attention_mask':torch.tensor(attention_mask).long(),'token_type_ids':torch.tensor(token_type_ids).long(),'edge':torch.tensor(example['edge1']).long()}

                #tokenized = self.tokenizer.encode_plus(example['x1'], truncation='only_first', padding='max_length', max_length=self.max_len)
            return {"text_id": example_id, 'x1': G}
        #================================================
        if isinstance(example['x1'], str):
            tokenized = self.tokenizer(example['x1'], padding='max_length', truncation=True, max_length=self.max_len)
        else:
    ...
    def __getitem__(self, index):
        ...
        #print(self._process_func(self.dataset[index]))
        #return error
