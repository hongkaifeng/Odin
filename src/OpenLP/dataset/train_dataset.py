...
class TrainHnDataset(Dataset):
    ...
    def create_some_example(self, text_encoding: List[int]):
    ...
    def process_fn(self, example, epoch, hashed_seed):
        ...
        #encoded_query = self.create_one_example(example['q_text'])
        x1=self.create_some_example(example['x1'])
        #encoded_query_n = [self.create_one_example(q_n) if q_n != [] else self.create_one_example([0]) for q_n in example['q_n_text']]
        #query_n_mask = [1 if q_n != [] else 0 for q_n in example['q_n_text']]
        # initial keys
        edge1=example['edge1']
        ...
        #print("pos")
        #print(pos_psg)
        encoded_key = self.create_one_example(pos_psg['x2'])
        #edge2=example['edge2']
        #edge2 = [self.create_one_example(k_n) if k_n != [] else self.create_one_example([0]) for k_n in pos_psg['edge2']]
        #key_n_mask = [1 if k_n != [] else 0 for k_n in pos_psg['k_n_text']]

        x2s.append([encoded_key])
        edge2s.append(pos_psg['edge2'])
        ...
        for neg_psg in negs:
            x2s.append([self.create_one_example(neg_psg['x2'])])
            edge2s.append(neg_psg['edge2'])
        ...
        #print({"x1": x1, "edge1": edge1, 'x2':x2s, 'edge2':edge2s})
        #return error
        return {"x1": x1, "edge1": edge1, 'x2':x2s, 'edge2':edge2s}
...
class EvalRerankDataset(Dataset):
    ...
    def process_fn(self, example):
        #print(example)
        #print("这里是eval dataset")
        #return error
        encoded_query = self.create_some_example(example['x1'])
        #print(encoded_query)
        #return error
        #x1={'x':encoded_query,'edge':example['edge1']}
        group_positives = example['positives']
        group_negatives = example['negatives']
        mask_key = {'x2': [101,100,102]}
        ...
        encoded_pos_key = [self.create_one_example(pos_psg['x2']) for pos_psg in group_positives]
        encoded_neg_key = [self.create_one_example(neg_psg['x2']) for neg_psg in group_negatives]
        encoded_keys = encoded_pos_key + encoded_neg_key
        #print(encoded_keys)
        label_mask = pos_mask + neg_mask + [self.pos_rerank_num, self.neg_rerank_num]
        #print(label_mask,"这里是EVAL DATASET LABEL")
        #x2={'x':encoded_keys,'edge':torch.tensor([[],[]]).long()}
        return({'x1':encoded_query,'edge1':example['edge1'],'x2':encoded_keys,'edge2':torch.tensor([[],[]]),'label':label_mask})
...
class TrainNCCDataset(Dataset):
    ...
    def process_fn(self, example):
        ...
        # print(example)
        # return error
        encoded_x1 = self.create_one_example(example['x1'])
        #encoded_x2 = self.create_one_example(example['x2'])
        return {"x1": encoded_x1, 'edge1': torch.tensor([example['edge1'][0], example['edge1'][1]]), 'label':example['label']}
